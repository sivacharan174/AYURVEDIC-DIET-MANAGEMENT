<?php
/**
 * Get Vikriti Assessment for a patient
...
if ($get_all) {
    $query = "SELECT * FROM vikriti_assessment WHERE patient_id = $patient_id ORDER BY created_at DESC";
    ...
    $query = "SELECT * FROM vikriti_assessment WHERE patient_id = $patient_id ORDER BY id DESC LIMIT 1";
    $result = mysqli_query($conn, $query);
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo json_encode([
            'success' => true,
            'exists' => false,
            'message' => 'No Vikriti data found for this patient'
        ]);
    }
}
