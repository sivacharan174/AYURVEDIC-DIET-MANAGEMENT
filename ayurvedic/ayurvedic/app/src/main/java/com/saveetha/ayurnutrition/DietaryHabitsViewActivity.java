...
/**
 * Displays all Dietary Habits records in RecyclerView cards.
 * Click on a card to see full details.
 */
public class DietaryHabitsViewActivity extends AppCompatActivity
        implements PatientDataHistoryAdapter.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView tvNoDataEmoji = findViewById(R.id.tvNoDataEmoji);
        TextView tvNoDataTitle = findViewById(R.id.tvNoDataTitle);

        if (tvTitle != null)
            tvTitle.setText("🍽️ Dietary Habits");
        if (tvSubtitle != null)
            tvSubtitle.setText("Tap any record to see details");
        if (tvNoDataEmoji != null)
            tvNoDataEmoji.setText("🍽️");
        if (tvNoDataTitle != null)
            tvNoDataTitle.setText("No dietary records yet");
        ...
        btnAddNew.setOnClickListener(v -> {
            Intent intent = new Intent(this, DietaryHabitsActivity.class);
            intent.putExtra("patient_id", patientId);
            startActivity(intent);
        });

        if (btnAddNew != null)
            btnAddNew.setText("➕ Add Dietary Record");
    ...
    private void addItem(JSONObject data) {
        try {
            ...
            item.setDataType("Dietary Habits");
            item.setDate(formatDate(data.optString("created_at", "")));

            String dietType = data.optString("diet_type", "");
            String mealCount = data.optString("meal_count", "");
            String summary = "";
            if (!dietType.isEmpty() && !dietType.equals("null")) {
                summary = dietType;
            }
            if (!mealCount.isEmpty() && !mealCount.equals("null")) {
                if (!summary.isEmpty())
                    summary += " | ";
                summary += mealCount + " meals/day";
            }
            if (summary.isEmpty())
                summary = "Dietary record";
    ...
    public void onItemClick(PatientDataHistoryItem item) {
        Intent intent = new Intent(this, DietaryHabitsDetailActivity.class);
